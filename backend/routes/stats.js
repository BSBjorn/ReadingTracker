...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const currentlyReadingResult = await pool.query(
      'SELECT COUNT(*) as count FROM books WHERE start_date IS NOT NULL AND end_date IS NULL'
    );
    ...
    const pagesReadResult = await pool.query(
      'SELECT COALESCE(SUM(pages), 0) as total FROM books WHERE end_date IS NOT NULL AND pages IS NOT NULL'
    );
